...
class TestVMDImporter(unittest.TestCase):
    ...
    def __compare_camera_states(self, state_before, state_after):
        ...
        if "rotation" in state_before and "rotation" in state_after:
            ...
            if (rot_before - rot_after).length > 0.001:
    ...
    def test_vmd_importer_initialization(self):
        ...
        if not vmd_files:
            self.skipTest("No VMD sample files found for testing")
    ...
    def test_mirror_mapper(self):
        ...
        # Test get method
        self.assertEqual(mirror_mapper.get("左腕"), "LeftArm", "Should map left arm correctly")
        self.assertEqual(mirror_mapper.get("右腕"), "RightArm", "Should map right arm correctly")
    ...
    def test_renamed_bone_mapper(self):
        ...
        mapper = RenamedBoneMapper(armature, rename_LR_bones=True)

        # Test Japanese naming should map to English LR format
        ...
        # Test with rename_LR_bones=False
        mapper = RenamedBoneMapper(armature, rename_LR_bones=False)

        # Test Japanese naming directly
        self.assertIsNotNone(mapper.get("左腕"), "Should find left arm bone without renaming")
        self.assertIsNotNone(mapper.get("右腕"), "Should find right arm bone without renaming")

        # Test with underscore
        mapper = RenamedBoneMapper(armature, rename_LR_bones=True, use_underscore=True)

        # Test initialization with init method
        empty_mapper = RenamedBoneMapper()
        empty_mapper.init(armature)
        self.assertIsNotNone(empty_mapper.get("左腕"), "Should initialize properly with init method")
        ...
        self.assertIsNotNone(mapper.get("arm.L"), "Should find bone using translator")
    ...
    def test_bone_converter(self):
        ...
        self.assertNotEqual(inverted_location, converted_location, "Inverted converter should produce different results")
    ...
    def test_bone_converter_pose_mode(self):
        ...
        self.assertAlmostEqual((inverted_location - test_location).length, 0.0, places=5, msg="Inverted conversion should be close to original value")
    ...
    def test_fn_bezier(self):
        ...
        self.assertAlmostEqual(mid_point.x, 0.5, places=5, msg="Split point should be at t=0.5")

        # Test evaluate
        result = bezier.evaluate(0.5)
        self.assertAlmostEqual(result.x, 0.5, places=5, msg="Evaluation at t=0.5 should have x=0.5")
    ...
    def test_vmd_import_to_camera(self):
        ...
        if not mmd_camera:
            self.skipTest("Could not create MMD camera for testing")
        ...
        for filepath in vmd_files[:1]:  # Test with first VMD file
            ...
            # Check if animation data was created
            self.assertIsNotNone(mmd_camera.animation_data, "Camera animation data should be created")
            if mmd_camera.animation_data:
                self.assertIsNotNone(mmd_camera.animation_data.action, "Camera action should be created")
            ...
            # We might not have changes if the VMD file doesn't have camera animations
    ...
    def test_vmd_import_to_lamp(self):
        ...
        if not mmd_lamp:
            self.skipTest("Could not create MMD lamp for testing")
        ...
        for filepath in vmd_files[:1]:  # Test with first VMD file
            ...
            # Check if animation data was created
            self.assertIsNotNone(mmd_lamp.animation_data, "Lamp animation data should be created")
            if mmd_lamp.animation_data:
                self.assertIsNotNone(mmd_lamp.animation_data.action, "Lamp action should be created")
            ...
            # We might not have changes if the VMD file doesn't have lamp animations
    ...
    def test_vmd_import_with_real_model(self):
        ...
        if not pmx_files or not vmd_files:
            self.skipTest("No PMX/PMD or VMD sample files available for testing")

        # Enable mmd_tools addon
        try:
            self.__enable_mmd_tools()
        except:
            self.skipTest("Could not enable mmd_tools for testing with real models")
        ...
        try:
            ...
            if not model_root:
                self.skipTest("Could not import model for real model import test")
    ...
    def test_vmd_import_parameters(self):
        ...
        if not vmd_files:
            self.skipTest("No VMD sample files found for testing parameters")
    ...
    def test_vmd_import_with_bone_mapper(self):
        ...
        if not vmd_files:
            self.skipTest("No VMD sample files found for testing bone mapper")

        # Create test armature
        armature = self.__create_test_armature()

        # Create custom bone mapper - maps everything to the first bone
        class CustomBoneMapper:
            def __init__(self, arm_obj):
                self.first_bone = arm_obj.pose.bones[0] if len(arm_obj.pose.bones) > 0 else None
            ...
            def get(self, bone_name, default=None):
                return self.first_bone or default

        # Create mapper instance
        mapper = CustomBoneMapper(armature)
        ...
        # All animation should be applied to the first bone
        if armature.animation_data and armature.animation_data.action:
            action = armature.animation_data.action
            first_bone_name = armature.pose.bones[0].name

            # Check if any FCurves target the first bone
            found_fcurve = False
            for fcurve in action.fcurves:
                if f'pose.bones["{first_bone_name}"]' in fcurve.data_path:
                    found_fcurve = True
                    break

            self.assertTrue(found_fcurve, "Custom mapper should apply animation to the first bone")
    ...
    def test_vmd_import_full_setup(self):
        ...
        if not vmd_files:
            self.skipTest("No VMD sample files found for full setup test")
        ...
        print(f"Full setup import resulted in:")
    ...
    def test_vmd_import_use_NLA(self):
        ...
        if not vmd_files:
            self.skipTest("No VMD sample files found for NLA test")
    ...
    def test_vmd_import_new_action_settings(self):
        ...
        if not vmd_files:
            self.skipTest("No VMD sample files found for new action test")
    ...
    def test_vmd_import_detect_changes(self):
        ...
        if not vmd_files:
            self.skipTest("No VMD sample files found for detect changes test")
        ...
        if mmd_camera:
            ...
            # Verify camera was processed (we can't easily test the actual detection logic without specific VMD data)
            if mmd_camera.animation_data:
                self.assertIsNotNone(mmd_camera.animation_data.action, "Camera action should be created with detect_camera_changes=True")
        ...
        if mmd_lamp:
            ...
            # Verify lamp was processed
            if mmd_lamp.animation_data:
                self.assertIsNotNone(mmd_lamp.animation_data.action, "Lamp action should be created with detect_lamp_changes=True")
